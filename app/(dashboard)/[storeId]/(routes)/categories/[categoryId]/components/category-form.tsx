...
const CategoryForm: React.FC<CategoryFormProps> = ({
  ...
  const { billboardId, storeId, categoryId } = useParams();
  const router = useRouter();

  const title = initialData ? "Edit category" : "Create category";
  const desciption = initialData ? "Edit category" : " Add a new category";
  ...
  const onDelete = async () => {
    try {
      ...
      toast.error(
        "Make sure you remove all producys using this category first."
      );
    } finally {
      setLoading(false);
      setOpen(false);
